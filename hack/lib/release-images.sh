...
build_docker_image() {
    ...
    if [ -f ${binary_path} ]; then
        local docker_build_path=${DOCKER_BUILD_BASE_IDR}/majk
        local docker_file_path=${docker_build_path}/Dockerfile.${bin_target}
        mkdir -p ${docker_build_path}
 
        local base_image="alpine:3.9"
...
# gen_yamls generates yaml files for the raven-controller-manager
gen_yamls() {
    ...
    (
        ...
        kustomize edit set image controller=$REPO/raven-controller-manager:$(get_version)
	)
    set +x
